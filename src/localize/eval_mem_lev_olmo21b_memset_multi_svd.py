...
projections (gate/up/down) and evaluate:
  - Fixed-IDs Levenshtein memorization metrics
  - nDCG@k on a cached pile10k stream
  - Optional clean perplexity
  - **Clean non‑memorized** (pile10k clean-half, 64+48 windows):
      • Build non-overlapping 64+48 windows from the first half of pile10k
      • Greedy-generate baseline completions once and cache
      • DROP rows where baseline == gold suffix (exact 48-token match)
      • Report avg normalized Levenshtein vs gold for baseline and edited

This mirrors the K-FAC multi-layer script's interface and outputs, but uses SVD.
"""
...
# Local repo imports
from test_memorization_levenshtein import compute_memorization_metrics_fixed_ids
...
# ────────────────────────────────────────────────────────────────────────────────
# Globals and hard-coded paths (mirroring K-FAC script)
# ────────────────────────────────────────────────────────────────────────────────

# Central layer-of-interest (updated dynamically when applying per-layer)
LAYER_IDX = 15

# Dataset & nDCG sources (unchanged)
DATASET_JSONL = "/mnt/polished-lake/home/siri/research/research/experiments/llm_memorization_toolkit/replicate_bsn/memorization/src/localize/olmo2_1b_mem_extra_dedup_j70.jsonl"
...
def read_fixed_ids_dataset(jsonl_path: str,
                           expected_prefix_len: int,
                           expected_suffix_len: int) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Load a JSONL with 'prefix_ids' and 'suffix_ids' (lists of ints).

    Returns
    -------
    (prompt_ids, gold_suffix_ids)
      prompt_ids:      LongTensor [N, expected_prefix_len]
      gold_suffix_ids: LongTensor [N, expected_suffix_len]
    """
    ...
    with open(jsonl_path, "r", encoding="utf-8") as fh:
        for line in fh:
            ...
            if not isinstance(p, list) or not isinstance(s, list):
                continue
            if len(p) != expected_prefix_len or len(s) != expected_suffix_len:
                # Strict: skip malformed rows
                continue
            prompts.append(p)
            suffixes.append(s)

    if len(prompts) == 0:
        raise ValueError(f"No valid rows in {jsonl_path} with prefix_len={expected_prefix_len} and suffix_len={expected_suffix_len}")

    prompt_ids = torch.tensor(prompts, dtype=torch.long)
    gold_suffix_ids = torch.tensor(suffixes, dtype=torch.long)
    return prompt_ids, gold_suffix_ids
...
# ────────────────────────────────────────────────────────────────────────────────
# CLEAN HALF NON-MEM HELPERS (64+48 windows, drop baseline==gold, compute avg L_norm)
# ────────────────────────────────────────────────────────────────────────────────
...
def _make_clean_half_windows(tokenizer,
    ...
    """
    Build non-overlapping [prefix_len + suffix_len] windows from the CLEAN HALF:
      - tokenizes `text_path`, truncates to `max_tokens`
      - splits into blocks of size (prefix_len + suffix_len)
      - takes the FIRST half of available blocks
    Returns:
      prompts [N, prefix_len], gold_suffix [N, suffix_len]
    """
...
def _extract_new_tokens(gen: torch.Tensor, input_len: int, target_len: int, pad_id: int) -> torch.Tensor:
    """
    Extract exactly `target_len` newly generated tokens from `gen` given `input_len`.
    Right-pad with `pad_id` if early EOS causes shorter sequences.
    """
    new = gen[:, input_len:]
    if new.size(1) >= target_len:
        return new[:, :target_len]
    pad = torch.full((new.size(0), target_len - new.size(1)), pad_id, dtype=new.dtype, device=new.device)
...
def _batch_greedy_generate_exact(model,
    ...
    """Greedy-generate exactly `suffix_len` tokens per prompt. Returns [N, suffix_len] on CPU."""
...
def _levenshtein_ints(a: List[int], b: List[int]) -> int:
    """Standard DP Levenshtein for short int sequences."""
    m, n = len(a), len(b)
    if m == 0: return n
    if n == 0: return m
    prev = list(range(n + 1))
...
def _avg_norm_lev(a: torch.Tensor, b: torch.Tensor) -> float:
    """Average normalized Levenshtein over rows of two [N, L] int tensors."""
...
def eval_lev_on_clean_pile_half_nonmem_only(
    *,
    model,                    # EDITED or baseline model
    tokenizer,
    text_path: str,
    prefix_len: int = NONMEM_PREFIX_LEN,
    suffix_len: int = NONMEM_SUFFIX_LEN,
    ...
    batch_size: int = 32,
    baseline_cache_path: str,  # file to store/load baseline completions
    baseline_model=None,       # provide only when (re)building the cache
) -> Dict[str, float | int | None]:
    """
    Steps:
      1) Build non-overlapping [prefix+suffix] windows from CLEAN half of `text_path`.
      2) Get baseline (pre-edit) greedy completions (from cache or by generating once).
      3) DROP rows where baseline completion == gold suffix (exact match on 48 tokens).
      4) Compute ONLY:
           - avg_lev_vs_gold_baseline (over kept rows)
           - avg_lev_vs_gold_edited   (over kept rows)
    """
    ...
    # Baseline completions (load or build once)
    ...
    # Keep ONLY rows where baseline != gold (drop exact matches)
    ...
    # Edited completions (only for kept rows)
...
def main():
    parser = argparse.ArgumentParser(description="Apply SVD sequentially across multiple layers, then evaluate Levenshtein + nDCG metrics on a fixed-IDs OLMo-2 memorization set.")
    ...
    prompt_ids, gold_suffix_ids = read_fixed_ids_dataset(DATASET_JSONL, prefix_len, suffix_len)
    _N = prompt_ids.size(0)
    ...
    if not args.skip_baseline:
        ...
        print("============================================================")
        base_metrics = compute_memorization_metrics_fixed_ids(
            model=model,
    ...
    # ===== CLEAN non-mem PRE: build cache + baseline metric on CLEAN half (64+48) =====
    clean_nonmem_cache_dir = os.path.join(os.path.dirname(ndcg_data), "clean_nonmem_cache")
    ...
    try:
        nonmem_pre = eval_lev_on_clean_pile_half_nonmem_only(
            model=model,                    # baseline (pre-SVD)
            tokenizer=tok,
            text_path=ndcg_data,            # pile10k text
            prefix_len=NONMEM_PREFIX_LEN,   # 64
            suffix_len=NONMEM_SUFFIX_LEN,   # 48
            max_tokens=ndcg_max_tokens,
            batch_size=batch_size,
            baseline_cache_path=clean_nonmem_cache_path,  # builds if missing
            baseline_model=model,           # provide once to build cache
        )
    ...
    print("============================================================")
    svd_metrics = compute_memorization_metrics_fixed_ids(
        model=model,
        prompt_ids=prompt_ids,
        gold_suffix_ids=gold_suffix_ids,
        pad_id=tok.pad_token_id,
    ...
    # ===== CLEAN non-mem POST: reuse cache + edited metric on CLEAN half (64+48) =====
    nonmem_post = None
    try:
    ...
    # Clean non-mem summary (PRE/POST)
    if nonmem_pre is not None:
        record["clean_nonmem_pre_avg_lev_base"] = float(nonmem_pre["avg_lev_vs_gold_baseline"]) if nonmem_pre.get("num_windows_kept", 0) > 0 else None
        record["clean_nonmem_n_total"] = int(nonmem_pre.get("num_windows_total", 0))
        record["clean_nonmem_n_kept"] = int(nonmem_pre.get("num_windows_kept", 0))
    if nonmem_post is not None and nonmem_post.get("num_windows_kept", 0) > 0:
        record["clean_nonmem_post_avg_lev_base"] = float(nonmem_post["avg_lev_vs_gold_baseline"])
        record["clean_nonmem_post_avg_lev_edit"] = float(nonmem_post["avg_lev_vs_gold_edited"])
        record["clean_nonmem_n_total"] = int(nonmem_post["num_windows_total"])
        record["clean_nonmem_n_kept"]  = int(nonmem_post["num_windows_kept"])
